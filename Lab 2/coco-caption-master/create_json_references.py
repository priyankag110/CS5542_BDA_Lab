...
class CocoAnnotations:
  ...
  def get_image_dict(self, img_name):
    image_hash = int(int(hashlib.sha256(img_name.encode()).hexdigest(), 16) % sys.maxsize)
  ...
  def dump_json(self, outfile):
    ...
    with io.open(outfile, 'w', encoding='utf-8') as fd:
      fd.write(pylab.unicode(json.dumps(res, ensure_ascii=True,sort_keys=True,indent=2,separators=(',', ': '))))
